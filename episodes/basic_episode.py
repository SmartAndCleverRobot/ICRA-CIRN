...
c2p_prob = json.load(open("/home/Newdisk/lixinting/zero-shot-data/data/c2p_prob.json"))
...
class BasicEpisode(Episode):
    ...
    def __init__(self, args, gpu_id, strict_done=False):
        ...
        self.room = None
        if args.zsd == True and args.model != 'ZeroZero':
            if args.split == "18/4":
                n = 97
            ...
            n = 101
            self.unseen_objects = []
        # glove embeddings for all the objs. 这段代码在Zero_object代码中没有用到
        self.objects = []
        with open("/home/Newdisk/lixinting/zero-shot-data/data/gcn/objects.txt") as f:
            objects = f.readlines()
            for o in objects:
                o = o.strip()
                if args.zsd == True and args.model != 'ZeroZero':
    ...
    # def rgb_state_for_agent(self):
    #     return self.environment.rgb
        ...
        if self.environment.controller.state in self.scene_states:
            if action["action"] != DONE:
                ...
                if self.partial_reward:
                    if args.model == "SelfAttention_test" or args.model == 'ZeroZero' or args.model == 'ZeroGCN':
