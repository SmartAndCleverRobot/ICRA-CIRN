...
class BaselineClipModel(torch.nn.Module):
    def __init__(self, args):
        ...
        super(BaselineClipModel, self).__init__()
    ...
    def embedding(self, state, action_embedding_input, target_txt):

        matching = torch.mul(state, target_txt)
        ...
        # target_embedding = target.reshape(1, self.num_cate * 49)
        ...
        image_embedding = state
        x = matching
        x = torch.cat((x, matching, action_embedding), dim=1)
    ...
    def forward(self, model_input, model_options):
        ...
        target_text = model_input.target_text

        x, image_embedding = self.embedding(state, action_probs, target_text)
