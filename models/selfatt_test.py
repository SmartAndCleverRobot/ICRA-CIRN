...
class SelfAttention_test(torch.nn.Module):
    def __init__(self, args):
        ...
        if args.zsd == True:
            if args.split == "18/4":
            ...
            elif args.split == "kitchen" or args.split == "living_room" or args.split == "bedroom" or args.split == "bathroom":
                if args.eval == False:
                    split = args.split
                else:
                    split = args.seen_split
                n = 0
                self.unseen_objects = []
                with open("/home/Newdisk/lixinting/zero-shot-data/data/gcn/objects.txt") as f: # 至少这里
                    ...
                    with open("/home/Newdisk/lixinting/zero-shot-data/data/gcn/"+split+".txt") as f: # 至少这里
                        objects = f.readlines()
                        objects = [i.split('\n')[0] for i in objects]
                        n = len(objects)
                        for i in objects_all:
                            if i not in objects:
                                self.unseen_objects.append(i)
        else:
            n = 101
            self.unseen_objects=[]
