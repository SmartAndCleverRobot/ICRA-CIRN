...
class OfflineControllerWithSmallRotationEvent:
    ...
    def __init__(self, last_action_success, scene_name, state=None, frame=None, objbb = None, rgb = None):
        self.metadata = {
...
class OfflineControllerWithSmallRotation(BaseController):
    ...
    def __init__(
        ...
        grid_size=0.25,
        fov=100,
        offline_data_dir= None, #"/mnt/6tb/mitchellw/data/living_room_offline_data",
    ...
    def reset(self, scene_name=None):
        ...
        if scene_name != self.scene_name:
            ...
            with open(
                os.path.join(
                    self.offline_data_dir, self.scene_name, self.objbb_file_name
                ),
                "r",
            ) as f1:
                self.curr_objs = json.load(f1)

            self.rgb = self.h5py.File(
                os.path.join(
                    '/home/Newdisk/lixinting/ECCV-VN-master-data/Data/AI2thor_offline_data_2.0.2/', self.scene_name, 'images.hdf5'
                ),
                'r',
            )            
    ...
    def objType_is_visible(self, objType):
        if self.using_raw_metadata:
            ...
            return objType in visible_objects
        else:
            statelist = []
            for k, v in self.metadata.items():
                if objType in k:
                    statelist += v
            return str(self.state) in statelist
    ...
    def _successful_event(self):
        return OfflineControllerWithSmallRotationEvent(
            self.last_action_success, self.scene_name, self.state, self.get_image(), self.get_objbb(), self.get_rgb()
        )
    ...
    def get_objbb(self):
        return self.curr_objs[str(self.state)]

    def get_rgb(self):
        try:
            a=str(self.state)
            return self.rgb[str(self.state)][:]
        except:
            print(self.scene_name)
